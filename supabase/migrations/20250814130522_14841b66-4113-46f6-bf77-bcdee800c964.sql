-- Fix security issues with updated policies and validation

-- 1. Update bookings table policies for better security
...
WITH CHECK (
  ...
  (auth.uid() IS NULL AND guest_email IS NOT NULL AND length(customer_name) >= 2)
);
...
USING (
  (auth.uid() IS NOT NULL AND user_id = auth.uid()) OR
  (auth.uid() IS NULL AND session_id IS NOT NULL AND length(session_id) >= 16)
);
...
-- 4. Add validation function for customer data
